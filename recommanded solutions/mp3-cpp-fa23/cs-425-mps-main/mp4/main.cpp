...
#include "../mp3/data_node/data_node_server.hpp"
#include "../mp3/name_node/name_node_server.hpp"
#include "../mp3/data_node/data_node_block_report_manager.hpp"
#include "../mp3/data_node/data_node_client.hpp"
#include "../mp3/miscellaneous/name_node_identifier.hpp"
#include "../mp3/miscellaneous/multi_read.hpp"
...
// Variables for MP 4
int worker_server_port = 20345;
pthread_t worker_server_thread;

// Variables for MP 3
const int NUM_REPLICAS = 2;
const int X = 2; //majority of ACK in writing
...
void setup_maple_juice_threads() {
	// launch worker server
	launch_worker_server(worker_server_port, worker_server_thread);
}
...
void parse_cmd_args(int argc, char** argv, 
	...
	while ((opt = getopt(argc, argv, "i:v:s:n:d:o:p:I:G:R:V:D:N:T:M:")) != -1) {
		switch (opt) {
			...
			case 'W': // worker server port                                                                                                 
				{
					std::string s = std::string(optarg);
					worker_server_port = convert_to_int(s, "Worker server port");
					break;
				}
			default: /* '?' */
