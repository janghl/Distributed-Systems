...
std::string mp3_directory = mp_directory + "/mp3";
std::string mp3_logs_directory = mp3_directory + "/test";
...
void pull_from_repo_on_all_machines(const std::vector<int>& machines) {
	std::string gitclone = "git clone https://" + github_user + ":" + github_token + "@gitlab.engr.illinois.edu/" + 
        ...
        for (const int machine_number : machines) {
                ...
                // pull/clone repo
                std::string update_local_command = "ssh " + machine_ssh + " \"(cd " + mp_directory + " && git pull && git checkout main && git pull) || " + gitclone + "\"";
                system(update_local_command.c_str());
        }
}

// Deletes all files in cs-425-mps/mp3/test (i.e. mp3_logs_directory) directory
void delete_logs(const std::vector<int>& machines) {
        for (const int machine_number : machines) {
                std::string machine_name = get_machine_name(machine_number);
                std::string machine_ssh = github_user + "@" + machine_name;
                std::string delete_logs_command = "ssh " + machine_ssh + "\" rm " + mp3_logs_directory + "/* && rm cs-425-mps/mp3/sdfs_files/* \"";
                system(delete_logs_command.c_str());
        }
}

// Creates directory cs-425-mps/mp3/test (i.e. mp3_logs_directory) 
void setup_logs(const std::vector<int>& machines) {
        for (const int machine_number : machines) {
                ...
                std::string create_test_dir_command = "ssh " + machine_ssh + " mkdir " + mp3_logs_directory;
                system(create_test_dir_command.c_str());
        }
}

std::string process_name = "sdfs";
std::string compile_program = "./compile.sh";

// Compiles mp3 on machine identified by machine_name
void* compile_process(void* machine_id_ptr) {
	...
	std::string machine_name = get_machine_name(machine_id);
        std::string machine_ssh = github_user + "@" + machine_name;
        std::string launch_process_command = "ssh " + machine_ssh + " \"cd " + mp3_directory + " && " + compile_program + "\"";
