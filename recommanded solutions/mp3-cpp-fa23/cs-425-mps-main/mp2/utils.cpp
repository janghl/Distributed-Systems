...
#ifdef DEBUG
	std::cout << "buffer: " << buffer << " buffer start: " << buffer_start << " total packet size: " << total_packet_size <<
			"buffer start - buffer: " << (buffer_start - buffer) << std::endl;
#endif
...
bool get_membership_and_protocol(int socket, uint32_t& client_generation_count, membership_protocol_options& client_gossip_protocol, 
	...
	char* org_buffer = buffer;
	uint32_t message_size = read_from_socket(socket, buffer, client_total_message_size);
	if (message_size != client_total_message_size - (2*sizeof(uint32_t))) {
#ifdef DEBUG
                std::cout << "Invalid message: got message of size " << message_size << " but expected size " 
			<< client_total_message_size << std::endl;
#endif
                return false;
        }
#endif
