...
#include "utils.hpp"
#include "extra_information_type.hpp"
...
typedef std::function<void(MemberInformation&)> node_failure_callback_function;
...
private:
	...
	// extra information
	ExtraInformationType* extra_information;
...
public:
	...
	MemberInformation(uint32_t machine_id, std::string machine_ip, uint32_t machine_version_number, uint32_t heartbeat_counter = 0, 
			...
			ExtraInformationType* extra_information = nullptr) :
	...
	MemberInformation(const MemberInformation& other) :
		machine_id(other.machine_id),
                machine_version_number(other.machine_version_number),
                machine_ip(other.machine_ip),
                ...
                machine_state(other.machine_state) {
			if (other.extra_information == nullptr) {
				this->extra_information = nullptr;
			} else {
				this->extra_information = new ExtraInformationType(*other.extra_information);
			}
	}

	MemberInformation(MemberInformation&& other) :
                machine_id(other.machine_id),
                machine_version_number(other.machine_version_number),
                machine_ip(other.machine_ip),
                heartbeat_counter(other.heartbeat_counter),
                ...
                last_update_time(other.last_update_time),
                machine_state(other.machine_state),
       		extra_information(other.extra_information) {
                        other.extra_information = nullptr;
        }

	MemberInformation& operator=(const MemberInformation& other) {
		this->machine_id = other.machine_id;
                this->machine_version_number = other.machine_version_number;
                this->machine_ip = other.machine_ip;
                this->heartbeat_counter = other.heartbeat_counter;
                this->incarnation_number = other.incarnation_number;
                this->last_update_time = other.last_update_time;
                this->machine_state = other.machine_state;
		if (other.extra_information == nullptr) {
			this->extra_information = nullptr;
		} else {
			this->extra_information = new ExtraInformationType(*other.extra_information);
	...
	uint32_t struct_serialized_size(bool gossip_with_suspect) {
		return sizeof(uint32_t) + // machine id 
			...
			this->extra_information->struct_serialized_size();
	}
	...
	char* serialize(char* buffer, bool gossip_with_suspect) const {
		...
		buffer = this->extra_information->serialize(buffer);
	...
	static const char* deserialize(const char* buffer, MemberInformation& member_information, bool gossip_with_suspect) {
		...
		member_information.extra_information = new ExtraInformationType;
		buffer = ExtraInformationType::deserialize(buffer, member_information.extra_information);
	...
	void merge_member_information(const MemberInformation& merge_source, unsigned long current_time, node_failure_callback_function node_failure_callback = nullptr) {
		std::lock_guard<std::mutex> lock(mtx);
...
#ifdef FAILED_PRINT
			std::cout << current_time << " FAILED (callback: " << (node_failure_callback ? "not-null" : "null") << ")" << std::endl;
			print_header(not is_protocol_gossip());
			this->print(not is_protocol_gossip());
#endif

			if (node_failure_callback) {
				node_failure_callback(*this);
			}
		...
		if (this->machine_state == MachineState::Suspected) {
			if (merge_source.incarnation_number > this->incarnation_number) {
				...
				this->extra_information->merge(merge_source.extra_information);
	...
	ExtraInformationType* get_extra_information() {
		return this->extra_information;
	}

	const ExtraInformationType* get_extra_information() const {
		return this->extra_information;
	}
...
private:
	...
	node_failure_callback_function node_failure_callback;
...
public:
	...
	MembershipListManager(uint32_t self_machine_id, uint32_t self_version_number) : 
		self_status(self_machine_id, get_machine_ip(self_machine_id), self_version_number, 0, 0, 0, new ExtraInformationType) {}

	MembershipListManager(std::vector<int>& machine_ids, uint32_t self_machine_id, uint32_t self_version_number) : 
		self_status(self_machine_id, get_machine_ip(self_machine_id), self_version_number, 0, get_current_time(), 0, new ExtraInformationType) {
			...
			for (size_t i = 0; i < machine_ids.size(); ++i) {
				if (self_status.is_same_member(machine_ids[i], self_version_number)) {
					...
					membership_list.emplace_back(machine_ids[i], get_machine_ip(machine_ids[i]), 0, 0, current_time, 0, 
							new ExtraInformationType);
				}
			}
		}
	...
	bool has_not_failed(uint32_t machine_id, bool lock_membership = false) {
		if (lock_membership) mtx.lock();
		if (self_status.get_machine_id() == machine_id && not self_status.has_failed()) {
			if (lock_membership) mtx.unlock();
			return true;
		}
		for(const auto& member : membership_list) {
			if(member.get_machine_id() == machine_id && not member.has_failed()){
				if (lock_membership) mtx.unlock();
				return true;
			}
		}
	...
	void init_self(uint32_t self_machine_id, uint32_t self_version_number, ExtraInformationType* extra_information = nullptr, 
				node_failure_callback_function node_failure_handler = nullptr) {
		if (extra_information == nullptr) {
			extra_information = new ExtraInformationType;
		}
		this->node_failure_callback = node_failure_handler;
		self_status.init(self_machine_id, get_machine_ip(self_machine_id), self_version_number, 0, 0, 0, extra_information);
	}
	...
	std::vector<MemberInformation>& get_membership_list() {
		return this->membership_list;
	}

	MemberInformation& self_get_status() {
		return this->self_status;
	}

	ExtraInformationType* get_self_extra_information() {
		return this->self_status.get_extra_information();
	}
	...
	// Returns lowest machine id present in membership list. The lowest machine id be same as the self_status.machine_id
	uint32_t get_lowest_node_id() {
		mtx.lock();
		uint32_t lowest_machine_id = this->self_status.get_machine_id();
		for (const auto& member : this->membership_list) {
			if (!member.has_failed() && lowest_machine_id > member.get_machine_id()) {
				lowest_machine_id = member.get_machine_id();
			}
		}
		mtx.unlock();

		return lowest_machine_id;
	}
	...
	void add_new_member(uint32_t machine_id, uint32_t machine_version_number) {
		...
		membership_list.emplace_back(machine_id, get_machine_ip(machine_id), machine_version_number, 0, get_current_time(), 0,
						new ExtraInformationType());
	...
	char* serialize_membership_list(uint32_t& membership_list_length, bool gossip_with_suspect) {
		...
		membership_list_length = this->self_status.struct_serialized_size(gossip_with_suspect);;
		for (int32_t i = 0; i < membership_list.size(); ++i) {
			membership_list[i].lock();
			membership_list_length += membership_list[i].struct_serialized_size(gossip_with_suspect);
		}
