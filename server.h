...
class Server{
	...
	void Initialize(){
		...
		bind_result = bind(server_socket, (struct sockaddr*)&myaddr, sizeof(struct sockaddr));
		if (bind_result < 0) {
			perror("bind failure!");
			return;
		}
	...
	void Connect() {
		...
		client_socket = accept(server_socket, (struct sockaddr*)&client_addr, &socket_length);
		if (client_socket < 0) {
			perror("acceptance error!");
			return;
		}
		char* client_ip = new char[INET_ADDRSTRLEN];
		inet_ntop(AF_INET, &client_addr.sin_addr, client_ip, INET_ADDRSTRLEN);		//network to host
