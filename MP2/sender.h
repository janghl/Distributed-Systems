...
private:
  ...
  std::string ListToString() const {
    ...
    for (const auto &[key, value] : membership_list_) {
      ss << key.host << "," << key.port << "," << key.time_stamp << "," << value.count
         << "," << value.local_time << "," << status_to_string_.at(value.status)
         << std::endl;
    }
    return retval;
  }
  ...
  std::map<NodeId, MembershipEntry> StringToList(const std::string &str) const {
    ...
    while (std::getline(std::stringstream(str), line)) {
      ...
      while (getline(std::stringstream(line), substr, ',')) {
        splitted.push_back(substr);
      }
      map.emplace(std::piecewise_construct,
                  std::forward_as_tuple(splitted[0], splitted[1], std::stol(splitted[2])),
                  std::forward_as_tuple(std::stoi(splitted[3]), std::stoi(splitted[4])), string_to_status_.at(splitted[5]));
    }
  }
  const int kTFail = 30;
